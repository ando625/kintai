...
最初に Seeder でダミーユーザーを作成した際、一度だけ認証メールが送信されます。
しかし、このメールは作成時点で送られるため、ログインする頃には 有効期限が切れている場合があります。


また、認証画面にある 「認証はこちらから」ボタンは MailHog を開くだけのボタンであり、
認証メールそのものを送る機能ではありません。
登録時に自動で送られる仕組みです。


そのため、Seeder作成後時間経過している場合、ログイン後の認証画面では
 **必ず「認証メールを再送する」を押して、新しく届いたメールから認証してください。**
